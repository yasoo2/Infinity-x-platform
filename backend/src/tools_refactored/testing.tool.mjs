...
async function generateAndRunTests({ filePath }) {
  try {
    console.log("[TestingTool] Starting test generation and execution for:", filePath);

    const fileContent = await readFile(filePath, "utf8");
    if (!fileContent) {
      return {
        success: false,
        error: `File not found or empty: ${filePath}`,
      };
    }
    ...
    if (!generationResult || !generationResult.testFilePath) {
      return {
        success: false,
        error: "Test generation failed to produce a file path.",
      };
    }
    ...
    console.error("[TestingTool] Test generation/execution failed:", error);
    return {
      success: false,
      error: `Failed to generate or run tests: ${error.message}`,
    };
  }
}
...
generateAndRunTests.metadata = {
  ...
  description:
    "Generates a new test file for a given source code file using AI, saves it, and then executes the tests, returning the results. This is the primary tool for ensuring code quality.",
  parameters: {
    type: "object",
    properties: {
      filePath: {
...
async function runExistingTests({ testFilePath }) {
  try {
    console.log("[TestingTool] Running existing tests at:", testFilePath);
    const results = await testingSystem.runTests(testFilePath);
    return { success: true, results };
  } catch (error) {
    console.error("[TestingTool] Test execution failed:", error);
    return {
      success: false,
...
runExistingTests.metadata = {
  ...
  description:
    "Executes an existing test suite located at a specific file path or directory and returns the results.",
  parameters: {
    type: "object",
    properties: {
      testFilePath: {
        type: "string",
        description:
          "The path to the test file or directory containing the tests to run.",
      },
    },
