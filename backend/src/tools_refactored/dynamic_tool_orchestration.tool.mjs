...
// of pre-written, highly parameterized functions to fulfill the request without LLM generation.
    ...
    async autoPlanAndExecute({ instruction, context }) {
        ...
        try {
            ...
            // Full website build orchestration
            if (has('build','website','site','ويب','موقع','بناء')) {
                const appName = context?.appName || 'joe-web-app';
                const directory = context?.directory || 'projects';
                const description = context?.description || 'Generated by JOE autonomous builder.';
                ...
                // Optional: discover packages and install
                if (has('حزمة','npm','باكدج','مكتبة','library','framework')) {
                    try {
                        const q = context?.pkgQuery || 'web framework ui';
                        const found = await tm.execute('discoverNpmPackages', { query: q, size: 3 });
                        push('Discover npm packages', { tool: 'discoverNpmPackages', output: found });
                        const top = (found?.items || [])[0]?.name;
                        if (top) {
                            const install = await tm.execute('updateDependencies', [top]);
                            push(`Install package ${top}`, { tool: 'updateDependencies', output: install });
                        }
                    } catch (eNpm) {
                        push('Package discovery/install skipped', { warning: eNpm?.message || String(eNpm) });
                    }
                }

                // Quality audits
                try {
                    const a11y = await tm.execute('auditAccessibility', { url: targetUrl });
                    push('Audit accessibility', { tool: 'auditAccessibility', output: a11y });
                } catch (e) { push('Audit accessibility failed', { warning: e?.message || String(e) }); }
                try {
                    const web = await tm.execute('auditWeb', { url: targetUrl });
                    push('Audit web performance/SEO', { tool: 'auditWeb', output: web });
                } catch (e) { push('Audit web failed', { warning: e?.message || String(e) }); }
                try {
                    const cross = await tm.execute('crossBrowserSmokeTest', { url: targetUrl, selector: 'body' });
                    push('Cross-browser smoke', { tool: 'crossBrowserSmokeTest', output: cross });
                } catch (e) { push('Cross-browser smoke failed', { warning: e?.message || String(e) }); }
                try {
                    const i18n = await tm.execute('auditI18n', { dir: context?.sourceDir || 'dashboard-x/src' });
                    push('Audit i18n/RTL', { tool: 'auditI18n', output: i18n });
                } catch (e) { push('Audit i18n/RTL failed', { warning: e?.message || String(e) }); }

                // Contract test if spec provided
                if (context?.openapi && context?.apiBaseUrl && context?.endpointPath) {
                    try {
                        const ct = await tm.execute('testEndpointAgainstSpec', { specPath: context.openapi, baseUrl: context.apiBaseUrl, path: context.endpointPath });
                        push('API contract test', { tool: 'testEndpointAgainstSpec', output: ct });
                    } catch (e) { push('API contract test failed', { warning: e?.message || String(e) }); }
                }

                // Resilience check (optional)
                try {
                    const rz = await tm.execute('simulateHttpFailures', { url: targetUrl, timeoutMs: 500 });
                    push('Resilience test', { tool: 'simulateHttpFailures', output: rz });
                } catch (e) { push('Resilience test failed', { warning: e?.message || String(e) }); }

                // Tests
                if (context?.testTarget) {
                    try {
                        const gt = await tm.execute('generateAndRunTests', { filePath: context.testTarget });
                        push('Generate and run tests', { tool: 'generateAndRunTests', output: gt });
                    } catch (e) { push('Generate/run tests failed', { warning: e?.message || String(e) }); }
                }

                // Deployment (placeholder tools available)
                if (context?.deploy?.projectPath) {
                    try {
                        const dep = await tm.execute('deployProject', { projectPath: context.deploy.projectPath, platform: context.deploy.platform || 'vercel' });
                        push('Deploy project', { tool: 'deployProject', output: dep });
                    } catch (e) { push('Deploy project failed', { warning: e?.message || String(e) }); }
                }
            }
